...
# mask_prob指的比较含糊，论文中，mask_prob用于说明每个token是mask span的起始索引的概率。
# 然而，在代码中，它mask_prob被用作可能的屏蔽向量的总百分比的上限
# 以后者为准，但是在短音频情况下，假设帧数为100时，num_masked_span为0.05*100/10=0.5，此时epsilon取值（若在0.5两侧）会造成不一样的结果
# 计算得到num_masked_span（选择作为起始索引的个数）可能会是1也可能是0，为0时导致np.random.choice选取0个数，引起index error
# 可以通过设置min_masks=1，就不会出现为0的情况
...
def _sample_negative_indices(
    ...
    mask_time_indices = (
        mask_time_indices.numpy().astype(np.bool) if mask_time_indices is not None else np.ones(features_shape, dtype=np.bool)
    )
