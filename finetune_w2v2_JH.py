...
class DataCollatorCTCWithPadding_JH:
    ...
    pad_to_multiple_of: Optional[int] = None
    pad_to_multiple_of_labels: Optional[int] = None
    split: Optional[float] = "x_train"

    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        batch = {}
        batch_size = features.__len__()
        input_features = []
        input_labels = []
        if "file" in features[0].keys() and "id" in features[0].keys() and "labels" in features[0].keys():
            for feature in features:
                seq = sio.loadmat(feature["file"])[self.split][feature["id"]-1]
                seq_normalized = (seq - np.mean(seq)) / np.sqrt(np.var(seq) + 1e-5)
                input_features.append(seq_normalized)
                seq_label = feature["labels"]
                input_labels.append(seq_label)
        else:
            raise ValueError(f" ['file','id','labels'] is required for collect func")
        batch["input_values"] = torch.tensor(np.stack(input_features),dtype=torch.float32)
        # zeros_pad = torch.zeros_like(input_values)
        # batch["input_values"] = torch.cat([input_values,zeros_pad],dim=1)
        # batch["input_values"] = torch.randn(4,400)
        batch["labels"] = torch.tensor(np.stack(input_labels),dtype=torch.long).view(batch_size)
...
class CTCTrainer(Trainer):
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        ...
        ignored_columns.remove("id") if "id" in ignored_columns else None
...
def main():
    ...
    if model_args.encoder_decoder_mode:
        ...
        model = Wav2Vec2ForSequenceClassification.from_pretrained(model_args.encoder_or_w2v2model_path)
        # state_dict = torch.load("/data2_from_58175/wav2vec2_output/fisher_joint/-checkpoint-118000/pytorch_model.bin",map_location="cpu")
        # model.load_state_dict(state_dict)

        # encoder_config = AutoConfig.from_pretrained("/data2_from_58175/huggingface/models/wav2vec2-base/config.json")
        # encoder_config.num_hidden_layers = 1
        # encoder_config.mask_time_prob = 0.0
        # encoder_config.mask_feature_prob = 0.0
        # encoder = Wav2Vec2ForCTC(encoder_config)
        # encoder = Wav2Vec2ForSequenceClassification(encoder_config)
        # state_dict = torch.load("/data2_from_58175/huggingface/models/wav2vec-large-xlsr-53-init-wenetspeech-vocabs/pytorch_model.bin",map_location="cpu")
        # encoder.load_state_dict(state_dict,strict=False)
        # model = encoder
    ...
    data_collator = DataCollatorCTCWithPadding_JH(processor=processor, padding=True, split="x_train")
    # 验证集不进行在线增强
    data_collator_eval = DataCollatorCTCWithPadding_JH(processor=processor, padding=True, split="x_test")

    config = {"command":sys.argv, **vars(model_args), **vars(data_args), **vars(training_args)}
